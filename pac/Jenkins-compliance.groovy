    ...
    stages {
        ...
        stage('Vulnerability scan and license checking') {
            ...
            steps {
                ...
                debricked-license debricked:license-report  "$DEBRICKED_CREDENTIALS_USR" "$DEBRICKED_CREDENTIALS_PSW" "$UPLOAD_ID" > compliance_${NETAPP_NAME}_"$GIT_COMMIT".report.txt || true
                '''
            }
        }
    }
    ...
    post {
        unsuccessful {
            ...
            emailext attachmentsPattern: '**/*.report.txt',
                ...
                mimeType: 'text/html',
                subject: "Evolved 5G - ${NETAPP_NAME} - FAIL - Compliance Analysis Result ${currentBuild.currentResult}: Job ${env.JOB_NAME}",
