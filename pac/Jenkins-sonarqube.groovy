        ...
        stage('Generate Markdown report and Upload reports to Artifactory') {
            ...
            steps {
                dir("${WORKSPACE}/") {
                    ...
                    docker run --rm -v "$WORKSPACE":$DOCKER_PATH ${PDF_GENERATOR_IMAGE_NAME}:${PDF_GENERATOR_VERSION} markdown-pdf -f A4 -b 1cm -s $DOCKER_PATH/utils/docker_generate_pdf/style.css -o $DOCKER_PATH/${REPORT_FILENAME}-${NETAPP_NAME_LOWER}-${GIT_NETAPP_BRANCH}.pdf $DOCKER_PATH/${REPORT_FILENAME}-${NETAPP_NAME_LOWER}-${GIT_NETAPP_BRANCH}.md
