        ...
        stage('Run the containers') {
            steps {
                dir ("${env.WORKSPACE}/${FOLDER_NAME}/"){
                    sh'''
                    make build
                    docker exec -it evolved5g_pilot_marketplace_laravel bash -c "composer install ; composer dump-autoload ; php artisan key:generate ; php artisan migrate ; php artisan db:seed; php artisan storage:link"
                    docker exec -it evolved5g_pilot_marketplace_laravel bash -c "npm install ; npm run prod"
                    '''
                }
            }
        }
